        ...
        <div key={group.dept} className="space-y-3">
          ...
          return (
            <Link
              key={member.id}
              href={`/team/${member.id}`}
              className="group bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex flex-col items-center text-center hover:shadow-[0_8px_32px_rgba(25,70,105,0.08)] hover:-translate-y-0.5 transition-all duration-200"
            >
              <div className="relative w-16 h-16 rounded-full overflow-hidden bg-[#F7F8FE] flex items-center justify-center flex-shrink-0">
                {override?.photo ? (
