...
export function BenefitAdmin({ employees, seedRequests }: { employees: Employee[]; seedRequests: SportBenefitRequest[] }) {
  const [selections, setSelections] = useState<Record<string, BenefitType>>({})
  const [requests, setRequests] = useState<SportBenefitRequest[]>([])

  const refresh = () => {
    setSelections(loadBenefitSelections(DEMO_BENEFIT_SELECTION))
    setRequests(loadSportRequests(seedRequests))
  }
  ...
  const benefitFor = (empId: string, fallback: BenefitType | null) => selections[empId] ?? fallback
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
          ...
          <h3 className="font-headline font-semibold text-navy">Sportovní benefit podle člověka</h3>
        </div>
        <div className="divide-y divide-slate-50">
          {employees.map((emp) => {
            const current = benefitFor(emp.id, emp.benefitType)
            return (
              <div key={emp.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400">{emp.department}{emp.employmentType === 'ICO' ? ' · OSVČ' : ''}</p>
                </div>
                <div className="flex items-center gap-2">
                  <select
                    value={current ?? ''}
                    onChange={(e) => saveBenefitSelection(emp.id, e.target.value as BenefitType)}
                    className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet focus:border-transparent bg-white"
                  >
                    <option value="" disabled>— vyber —</option>
                    <option value="MULTISPORT">Multisport karta</option>
                    <option value="SPORT_CONTRIBUTION">Příspěvek na sport ({fmt(SPORT_CONTRIBUTION_AMOUNT)}/měs.)</option>
                  </select>
                </div>
              </div>
            )
      ...
      {/* Čekající žádosti o příspěvek */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
          <h3 className="font-headline font-semibold text-navy">Čekající žádosti o příspěvek</h3>
          {pending.length > 0 && (
              ...
              <div key={r.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-slate-600 flex items-center gap-1.5">
                    ...
                    {r.receiptName} — {MONTH_NAMES_CZ[r.month - 1]} {r.year}
                  </p>
            ...
            {processed.map((r) => (
              <div key={r.id} className="px-6 py-3 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm text-navy">{r.employeeName} — {MONTH_NAMES_CZ[r.month - 1]} {r.year}</p>
                  <p className="text-xs text-slate-400">{r.receiptName}</p>
                </div>
                <div className="flex items-center gap-3 flex-shrink-0">
                  <span className="text-sm font-semibold text-navy">{fmt(SPORT_CONTRIBUTION_AMOUNT)}</span>
                  {r.status === 'APPROVED' ? (
