...
const adminItems = [
  ...
  { label: 'Doklady k proplacení', href: '/admin/vydaje', icon: Receipt },
  { label: 'Přehled faktur OSVČ', href: '/admin/faktury', icon: FileText },
...
export function Sidebar({ isAdmin, isTL, userName, userEmail, onClose }: SidebarProps) {
  ...
  return (
    <div className="relative flex flex-col h-full bg-white border-r border-slate-100">
      {/* Logo header */}
      <div className="relative flex items-center justify-between border-b border-slate-100 px-5 py-5">
      ...
      {/* User footer */}
      <div className="py-3 border-t border-slate-100 px-3">
        <div className="flex items-center mb-0.5 gap-3 px-3 py-2.5 rounded-xl">
        ...
        <button
          ...
          className="flex items-center w-full gap-3 px-3 py-2.5 rounded-full text-sm font-medium text-slate-400 hover:bg-slate-50 hover:text-navy transition-all duration-150"
        >
